<?php
class CommentList extends TPage{
    ...
    {   
        ...
        return CommentRecord::finder()->count($criteria);
    }
    ...
    protected function getSearchAdditonalCondition($t1=false)
    {
        $con = "";
        $t = $t1;
        if(strlen($this->subject1->Text)>0)
        {
            $con .= (($t?" AND ":"")."(title like '%". $this->subject1->Text . "%')");
            $t = true;
        }
        if(strlen($this->name->Text)>0)
        {
            $con .= (($t?" AND ":"")."(name like '%". $this->name->Text . "%')");
            $t = true;
        }
