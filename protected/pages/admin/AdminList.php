...
class AdminList extends TPage{
    ...
    {   
        ...
        $criteria->Condition = "_del= :_del";
        $criteria->Parameters[':_del'] = 0;
        return AccountRecord::finder()->count($criteria);
    }
    
    protected function getData($offset,$limit){
            
        $criteria = new TActiveRecordCriteria;
        $criteria->Condition = "_del= :_del" .$this->getSearchAdditionalCondition(true);
        $criteria->Parameters[':_del'] = 0;
        $criteria->Limit = $limit;
        $criteria->Offset = $offset;
        $c = AccountRecord::finder()->findAll($criteria);
        return $c;
    }
    ...
    protected function getSearchAdditionalCondition($t1 = false)
    {
        ...
        else if(strlen($this->txtSearchFamily->Text) > 0)
        {
            $con .= (($t? " AND ":"")."(family like '%" . $this->txtSearchFamily->Text . "%')");
            $t = true;
        }
        else if(strlen($this->txtSearchUser->Text) > 0)
        {
            $con .= (($t? " AND ":"")."username like '%" . $this->textSearchUser->Text . "%' ");
            $t = true;
        }
