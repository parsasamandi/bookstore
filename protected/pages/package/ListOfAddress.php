<?php
class ListOfAddress extends TPage{
    public function onInit($param)
    {
        if(empty($_SESSION["session_user_digi"]))
		{
            $_SESSION['AddressList'] = "AddressList is not empty";
    ...
    {   
        ...
        $criteria->Condition = "book_id in (select book_id from jazire_ketab.`like` where u_id=:u_id)" . $this->getSearchAdditonalCondition(true); 
	...
	{
        $criteria = new TActiveRecordCriteria;
        $criteria->Condition = "_del= :_del and u_id=:u_id" . $this->getSearchAdditonalCondition2(true); 
